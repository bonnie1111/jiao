...
<div class="fenye">
			<table class="footTable">
				<tr>
					<td colspan="5">共${activityInfo.total } 条记录 每页${activityInfo.size } 条
						第${activityInfo.pageNum } 页 <a href="javascript:void(0)"
						onclick="pageSelect(1)">首页</a> <c:choose>
							<c:when test="${activityInfo.hasPreviousPage }">
								<a href="javascript:void(0)"
									onclick="pageSelect(${activityInfo.prePage})">上一页</a>
							</c:when>
							<c:otherwise>
							...
							<c:when test="${activityInfo.hasNextPage }">
								<a href="javascript:void(0)"
									onclick="pageSelect(${activityInfo.nextPage})">下一页</a>
							</c:when>
							<c:otherwise>
								<font color="#ABA8AB">下一页</font>
							</c:otherwise>
						</c:choose> <a href="javascript:void(0)"
						onclick="pageSelect(${activityInfo.lastPage })">尾页</a>
					</td>
				</tr>
...
<script type="text/javascript">
	...
	function pageSelect(curPage){
		var url = "activity/findActivitiesByCustId"+custId+"/"+curPage;
		$('.layui-body').load(url);
	}
	$(function(){
		// 显示删除模态框
		...
		$('.btn-primary').click(function(){
			var url = "activities/deleteActivityById"+atvId+"/1";
			$('.layui-body').load(url);
			$('#deleteModal').hide();
		})
		// 点击新增显示模态框
		...
		$('.editUser').click(function(){
			atvId = $(this).attr("value");
			var url = "activity/findActivitiesById"+atvId;
		...
		$('button:contains(返回)').on('click',function(){
			$(".layui-nav-item dd:contains('客户信息查询')").tragger('click');
