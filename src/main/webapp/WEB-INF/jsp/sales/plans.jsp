		    ...
		    <tr>
		     <td>${chance.chcId }</td>
		      ...
		      <c:if test="${chance.chcStatus==1 }">
						<td>等待开发</td>
						<td> <i value="${chance.chcId }" class="layui-icon layui-icon-loading"></i></td>
						
						</c:if>
						<c:if test="${chance.chcStatus==2 }">
						<td>正在开发</td>
						 <td><i value="${chance.chcId }" class="layui-icon layui-icon-fonts-code"></i> </td>
						</c:if>
						<c:if test="${chance.chcStatus==3 }">
						<td>开发成功</td>
						<td> <i value="${chance.chcId }" class="layui-icon layui-icon-face-smile"></i> </td> 
						</c:if>
		    </tr> 
		    </c:forEach>		        
         </tbody>
...
<div class="fenye">
			<table class="footTable">
				<tr>
					<td colspan="5">共${chanceInfo.total } 条记录 每页${chanceInfo.size } 条
						第${chanceInfo.pageNum } 页 <a href="javascript:void(0)"
...
<script type="text/javascript">
	function pageSelect(curPge){
		var url = "plans/findPlansByUserId"+curPage;
		$('.layui-body').load(url);
	}
	...
	$(function(){
		...
		$('.search').click(function(){
			...
			var url = "plans/findChanceByUserNameAndRegion/1";
			$('.layui-body').load(url,{region:region});
		})
		$('.layui-icon-loading').click(function(){
			chanceId = $(this).attr("value");
			$('.layui-body').load('plans/toPlanAdd/'+chanceId);  
		})
		$('.layui-icon-fonts-code').click(function(){
			chanceId = $(this).attr("value");
			$('.layui-body').load('plans/toPlanEdit/'+chanceId);  
		})
		$('.layui-icon-face-smile').click(function(){
			chanceId = $(this).attr("value");
			$('.layui-body').load('plans/toPlanDetail/'+chanceId);  
		})
	})
</script>