    ...
    <div class="modal-content">
       <form>
					<div class="modal-body">
						...
						<div class="form-group">
							<label>服务处理过程:</label>
							<input type="textarea" name="servehandle" style="height:60px" class="form-control">
						</div>
					</div>
				</form>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$('button:contains(保存)').on('click',function(){
			var serveType = $("selected[name=serveType] option:selected").val();
			var title = $("input[name=title]").val();
			var cname = $("input[name=cname]").val();
			var status = $("select[name=status] option:selected").val();
			var dispose =  $("input[name=dispose]").val();
			var request = $("input[name=request]").val();
			var servehandle = $("input[name=servehandle]").val();
			var data = {
					svrId:servId,
					svrType:serveType,
					svrTitle:title,
		...
		$('.editUser').click(function(){
			...
			$.get(url,function(data){
				 serveType = $("select[name=serveType]").val(data.svrType);
				 ...
				 servehandle=$("input[name=servehandle]").val(data.svrHandle);
			});
			$('#toAdd').show();
		})
		// 点击更新
