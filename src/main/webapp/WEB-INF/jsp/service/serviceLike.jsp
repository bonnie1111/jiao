		 ...
		 <thead>
		  ...
		  <c:forEach items="${serveinfo.list }" var="service">
		    <tr>	
		      ...
		      <td> 
		      	<i value="service.svrId" class="layui-icon layui-icon-edit editUser" title="编辑"></i> 
      	        <i avlue="service.svrId" class="layui-icon layui-icon-ok updata" title="更新"></i> 
		      </td>
		      </c:if>
    ...
    <div class="modal-content">
       <form>
					<div class="modal-body">
						...
						<div class="form-group">
							<label>服务处理过程:</label>
							<input type="textarea" name="handle" style="height:60px" class="form-control">
						</div>
					</div>
				</form>
...
<script type="text/javascript">
	function pageSelect(curPge){
	$('.layui-body').load("service/findServiceLike/"+curPage);
	};
	$(function(){
		var servId;
		$('.layui-icon-face-smile').click(function(){
			 servId = $(this).attr("value");
			var url = "service/toServiceDetail/"+servId;
			 $('.layui-body').load(url);  
		})
		// 点击新增显示模态框
		$('#add').click(function(){
		...
		$('button:contains(保存)').on('click',function(){
			var servetype = $("select[name=servetype] option:selected").val();
		...
		$('.editUser').click(function(){
			...
			$.get(url,function(data){
				 servetype = $("select[name=servetype] option:selected").val(data.svrType);
				 title= $("input[name=title]").val(data.svrTitle);
				 cname=$("input[name=cname]").val(data.svrCustName);
				 status=$("select[name=status]").val(data.svrStatus);
				 dispose=$("input[name=dispose]").val(data.svrDispose);
				 request=$("input[name=request]").val(data.svrRequest);
				 handle=$("input[name=handle]").val(data.svrHandle);
			});
			$('#toAdd').show();
		})
		// 点击更新
		...
		$('.search').click(function(){
			...
			var url = "service/findServiceLike/";
